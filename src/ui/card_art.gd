...
## Deterministic abstract placeholder art.
##
## The art is generated from the definition's art block, so replacing it later
## with real illustration changes nothing about the card's mechanics or
## identity. Cost, stats and rules text are never drawn here — they always live
## in real controls outside the art, per the template direction.
